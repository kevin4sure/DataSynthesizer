...
def usefulness_minus_target(k, num_attributes, num_tuples, target_usefulness=5, epsilon=0.0):
...
def calculate_k(num_attributes, num_tuples, target_usefulness=4, epsilon=0.0):
...
def exponential_mechanism(dataset, mutual_info_list, epsilon=0.0):
...
def get_noisy_distribution_of_attributes(attributes, encoded_dataset, epsilon=0.0):
...
def construct_noisy_conditional_distributions(bayesian_network, encoded_dataset, epsilon=0.0):
